...
await page.waitForTimeout(300)
check('1号线 2 站', (await page.locator('.station-marker').count()) === 2)
...
await page.mouse.click(st1[0], st1[1])
await page.waitForTimeout(300)
check('点击已有站直接连入（换乘站出现）', (await page.locator('.st-icon.transfer, .st-dot.transfer').count()) === 1)
check('没有多放新站（仍 3 站）', (await page.locator('.station-marker').count()) === 3)
...
await page.waitForTimeout(300)
check('移出后换乘消失', (await page.locator('.st-icon.transfer, .st-dot.transfer').count()) === 0)
check('站点仍在（1号线上）', (await page.locator('.station-marker').count()) === 3)
await page.locator('.selection-bar .btn', { hasText: '✖️' }).last().click()
await page.waitForTimeout(200)

console.log('4. 线路时速设置')
const line1 = page.locator('.line-card', { hasText: '1号线' })
check('默认时速 80 高亮', await line1.locator('.pill-speed.pill-on').filter({ hasText: '80' }).isVisible())
await line1.locator('.pill-speed', { hasText: '100' }).click()
await page.waitForTimeout(200)
check('点击后 100 高亮', await line1.locator('.pill-speed.pill-on').filter({ hasText: '100' }).isVisible())
...
check('线卡时速设置保持 100', await line1.locator('.pill-speed.pill-on').filter({ hasText: '100' }).isVisible())
